...
namespace TBEasyWebCam
{
    public class EasyWebCam : MonoBehaviour
    {
        ...
        {
            int sourceId = 0;
            // Get the source ID from PlayerPrefs for all platforms
            string sourceName = PlayerPrefs.GetString("camera");

            WebCamDevice[] availableSources = WebCamTexture.devices;
            if (!string.IsNullOrEmpty(sourceName))
            {
                for (int i = 0; i < availableSources.Length; i++)
                {
                    if (sourceName == availableSources[i].name)
                    {
                        sourceId = i;
                        break;
                    }
            ...
            Debug.LogError("QR Camera: " + sourceName + ", id: " + sourceId);
#if UNITY_EDITOR
    ...
    if (easyWebCamInterface != null) {
        // Set the camera source ID before playing
        easyWebCamInterface.SwitchCamera(sourceId);
